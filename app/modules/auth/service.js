...
const { sendMail, getWelcomeEmailHtml, getWelcomeSubscriberEmailHtml } = require('../../utils/sendEmailGraph');
...
const createSubscriber = async (data) => {
  ...
  const activationLink = `https://mybizsolutions.us/login`;
  const welcomeHtml = getWelcomeSubscriberEmailHtml(user, activationLink, password);
  await sendMail({
    to: [user.email],
    subject: 'Welcome to MyBizSolutions!',
    html: welcomeHtml
